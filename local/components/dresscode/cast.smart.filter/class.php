		...
		if ($apply)
		{
			...
			foreach($this->arResult["ITEMS"] as $PID => $arItem)
			{
				...
				if ($smartPart)
				{
					...
					$smartParts[$arItem["ID"]] = $smartPart;
				}
			}
			
			//echo '<pre>';print_r($smartParts);echo '</pre>';
			//echo str_replace("#SMART_FILTER_PATH#", implode("/", $this->encodeSmartParts($smartParts)), $url);
			
			foreach($this->arResult["ITEMS"] as $PID => &$arItem)
			{
				if(!array_key_exists($PID, $smartParts)){
					foreach($arItem["VALUES"] as $key => &$ar)
					{
						if((!$ar["CHECKED"] || $ar["CONTROL_ID"] !== $checkedControlId) && strlen($ar["URL_ID"])){						
							$smartPart_temp = $smartParts;
							$smartPart_temp[$PID][] = $ar["URL_ID"];
							if ($arItem["CODE"])
								array_unshift($smartPart_temp[$PID], toLower($arItem["CODE"]));
							else
								array_unshift($smartPart_temp[$PID], $arItem["ID"]);
							ksort($smartPart_temp, SORT_NUMERIC); 
							$ar["URL_FOR_FILTER"] = str_replace("#SMART_FILTER_PATH#", implode("/", $this->encodeSmartParts($smartPart_temp)), $url);
							unset($smartPart_temp);	
						}
					}
				}
			}	

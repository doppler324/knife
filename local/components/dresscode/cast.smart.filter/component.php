	...
	if(!empty($arResult["ITEMS"]))
	{
		if ($this->facet->isValid())
		{
			...
			$arResult["FACET_FILTER"] = array(
				"ACTIVE_DATE" => "Y",
				"CHECK_PERMISSIONS" => "Y",
				"?TAGS" => $arParams["TAG_NAME"]
			);

			if(!empty($arParams["FILTER_ADD_PROPERTY_NAME"]) && !empty($arParams["FILTER_ADD_PROPERTY_VALUE"])){
				$arResult["FACET_FILTER"]["PROPERTY_".$arParams["FILTER_ADD_PROPERTY_NAME"]] = $arParams["FILTER_ADD_PROPERTY_VALUE"];
			}
		...
		else
		{
			...
			if(!empty($arParams["FILTER_ADD_PROPERTY_NAME"]) && !empty($arParams["FILTER_ADD_PROPERTY_VALUE"])){
				$arElementFilter["PROPERTY_".$arParams["FILTER_ADD_PROPERTY_NAME"]] = $arParams["FILTER_ADD_PROPERTY_VALUE"];
			}
	...
	foreach($arItem["VALUES"] as $key => $ar)
	{
		...
		)
		{
			...
			elseif($_CHECK[$ar["CONTROL_NAME"]] == $ar["HTML_VALUE"])
			{
				...
				$arResult['CHECKED'][$PID]['FILTER_HINT'] = $arResult["ITEMS"][$PID]['FILTER_HINT'];
				$arResult['CHECKED'][$PID]['VALUES'][] = $arResult["ITEMS"][$PID]["VALUES"][$key]['FACET_VALUE'];
				$arResult['CHECKED'][$PID]['VALUE'][] = $arResult["ITEMS"][$PID]["VALUES"][$key]['VALUE'];
				$arResult["ITEMS"][$PID]["DISPLAY_EXPANDED"] = "Y";
			...
			{
				$arResult["ITEMS"][$PID]["VALUES"][$key]["CHECKED"] = true;
				$arResult['CHECKED'][] = array($arResult["ITEMS"][$PID]['FILTER_HINT'], $arResult["ITEMS"][$PID]["VALUES"][$key]);
				$arResult["ITEMS"][$PID]["DISPLAY_EXPANDED"] = "Y";
				$allCHECKED[$PID][$ar["VALUE"]] = true;
				if ($arResult["FACET_FILTER"])
				{

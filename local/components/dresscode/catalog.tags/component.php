<?
	...
	if($this->StartResultCache($arParams["CACHE_TIME"], serialize($cacheID))){
		...
		while($obNextProduct = $rsProducts->GetNextElement()){
			...
			/* $arSectionFilter = array("IBLOCK_ID" => $arParams["IBLOCK_ID"], "=ID" => $arProduct["IBLOCK_SECTION_ID"]);
			$rsSection = CIBlockSection::GetList(array(), $arSectionFilter, false, array("CODE"));
			//check section info result
			if($arSection = $rsSection->GetNext()){
				$currentSectionCode = $arSection["CODE"];
			} */
			//check tags
			if(!empty($arProduct["TAGS"])){
				...
				foreach($arTags as $inx => $tagName){
					...
					//tag info
					$arTag = array("LINK" => $tagPath."tag/".$tagCode."/", "NAME" => DwItemInfo::mb_ucfirst($tagName), "CODE" => $tagCode, "COUNTER" => 1);

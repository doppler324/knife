			...
			<?if(!empty($arResult["PRICE"])):?>
				...
				<?else:?>
					<a class="price changePrice">
						<?if(!empty($arResult["PRICE"]["DISCOUNT"])):?>
							<span class="priceBlock">
								<span class="oldPriceLabel"><s class="discount">Цена: <?=CCurrencyLang::CurrencyFormat($arResult["PRICE"]["RESULT_PRICE"]["BASE_PRICE"], $arResult["EXTRA_SETTINGS"]["CURRENCY"], true)?></s></span>
								<?if(!empty($arResult["PRICE"]["RESULT_PRICE"]["DISCOUNT"])):?>
									<span class="oldPriceLabel"><?=GetMessage("OLD_PRICE_DIFFERENCE_LABEL")?> <span class="economy"><?=CCurrencyLang::CurrencyFormat($arResult["PRICE"]["RESULT_PRICE"]["DISCOUNT"], $arResult["EXTRA_SETTINGS"]["CURRENCY"], true)?></span></span>
								<?endif;?>
							</span>
						...
						<span class="priceContainer">
							<span class="priceVal">Цена: <?=CCurrencyLang::CurrencyFormat($arResult["PRICE"]["DISCOUNT_PRICE"], $arResult["EXTRA_SETTINGS"]["CURRENCY"], true)?></span>
							<?if($arParams["HIDE_MEASURES"] != "Y" && !empty($arResult["EXTRA_SETTINGS"]["MEASURES"][$arResult["CATALOG_MEASURE"]]["SYMBOL_RUS"])):?>
								<span class="measure"> / <?=$arResult["EXTRA_SETTINGS"]["MEASURES"][$arResult["CATALOG_MEASURE"]]["SYMBOL_RUS"]?></span>
							<?endif;?>
						</span>
				...
				<?if(!empty($arResult["PRICE"])):?>
					<?if($arResult["CATALOG_AVAILABLE"] != "Y"):?>
						<?if($arResult["CATALOG_SUBSCRIBE"] == "Y"):?>
							<a onclick="ym(21192229,'reachGoal','CLICK_OFFER');" target="_blank" href="/red.php?red=<?=urlencode('https://ad.admitad.com/g/17lxnyyakg5082102477c109c80522/?subid=' . $arResult['ID'] . '&ulp=' . $arResult['PROPERTIES']['URL_PRODUCT']['VALUE'])?>" class="addCart subscribe changeID changeQty changeCart" data-id="<?=$arResult["ID"]?>" data-quantity="<?=$arResult["EXTRA_SETTINGS"]["BASKET_STEP"]?>"><span><img src="<?=SITE_TEMPLATE_PATH?>/images/subscribe.png" alt="<?=GetMessage("SUBSCRIBE_LABEL")?>" class="icon">Подробнее</span></a>
						<?else:?>
							<a onclick="ym(21192229,'reachGoal','CLICK_OFFER');" target="_blank" href="/red.php?red=<?=urlencode('https://ad.admitad.com/g/17lxnyyakg5082102477c109c80522/?subid=' . $arResult['ID'] . '&ulp=' . $arResult['PROPERTIES']['URL_PRODUCT']['VALUE'])?>" class="addCart changeID changeQty changeCart disabled" data-id="<?=$arResult["ID"]?>" data-quantity="<?=$arResult["EXTRA_SETTINGS"]["BASKET_STEP"]?>"><span><img src="<?=SITE_TEMPLATE_PATH?>/images/incart.png" alt="<?=GetMessage("ADDCART_LABEL")?>" class="icon">Подробнее</span></a>
						<?endif;?>
					<?else:?>
						<a onclick="ym(21192229,'reachGoal','CLICK_OFFER');" target="_blank" href="/red.php?red=<?=urlencode('https://ad.admitad.com/g/17lxnyyakg5082102477c109c80522/?subid=' . $arResult['ID'] . '&ulp=' . $arResult['PROPERTIES']['URL_PRODUCT']['VALUE'])?>" class="addCart changeID changeQty changeCart" data-id="<?=$arResult["ID"]?>" data-quantity="<?=$arResult["EXTRA_SETTINGS"]["BASKET_STEP"]?>"><span><img src="<?=SITE_TEMPLATE_PATH?>/images/incart.png" alt="<?=GetMessage("ADDCART_LABEL")?>" class="icon">Подробнее</span></a>
					<?endif;?>
				<?else:?>
					<a onclick="ym(21192229,'reachGoal','CLICK_OFFER');" target="_blank" href="/red.php?red=<?=urlencode('https://ad.admitad.com/g/17lxnyyakg5082102477c109c80522/?subid=' . $arResult['ID'] . '&ulp=' . $arResult['PROPERTIES']['URL_PRODUCT']['VALUE'])?>" class="addCart changeID changeQty changeCart disabled requestPrice" data-id="<?=$arResult["ID"]?>" data-quantity="<?=$arResult["EXTRA_SETTINGS"]["BASKET_STEP"]?>"><span><img src="<?=SITE_TEMPLATE_PATH?>/images/request.png" alt="<?=GetMessage("REQUEST_PRICE_BUTTON_LABEL")?>" class="icon">Подробнее</span></a>
				<?endif;?>
				
			</div>
			<div class="mobileFastBackContainer row columnRow">
				<a onclick="ym(21192229,'reachGoal','CLICK_OFFER');" target="_blank" href="/red.php?red=<?=urlencode('https://ad.admitad.com/g/17lxnyyakg5082102477c109c80522/?subid=' . $arResult['ID'] . '&ulp=' . $arResult['PROPERTIES']['URL_PRODUCT']['VALUE'])?>" class="fastBack label changeID<?if(empty($arResult["PRICE"]) || $arResult["CATALOG_AVAILABLE"] != "Y"):?> disabled<?endif;?>" data-id="<?=$arResult["ID"]?>"><img src="<?=SITE_TEMPLATE_PATH?>/images/fastBack.png" alt="<?=GetMessage("FASTBACK_LABEL")?>" class="icon"><?=GetMessage("FASTBACK_LABEL")?></a>
			</div>
		</div>
	</div>
</div>

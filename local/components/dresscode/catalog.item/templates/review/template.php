...
<div id="<?=$this->GetEditAreaId($arResult["ID"]);?>">
	<?$this->SetViewTarget("after_breadcrumb_container");?>
		<h1 class="changeName"><?=$APPLICATION->GetTitle(false) . (Regions::is_region() ? " в " . Regions::get_name_case_region(5) : "");?></h1>
	<?$this->EndViewTarget();?>
	<div id="catalogElement" class="item<?if(!empty($arResult["SKU_OFFERS"])):?> elementSku<?endif;?>" data-product-iblock-id="<?=$arParams["IBLOCK_ID"]?>" data-from-cache="<?=$arResult["FROM_CACHE"]?>" data-convert-currency="<?=$arParams["CONVERT_CURRENCY"]?>" data-currency-id="<?=$arParams["CURRENCY_ID"]?>" data-hide-not-available="<?=$arParams["HIDE_NOT_AVAILABLE"]?>" data-currency="<?=$arResult["EXTRA_SETTINGS"]["CURRENCY"]?>" data-product-id="<?=!empty($arResult["~ID"]) ? $arResult["~ID"] : $arResult["ID"]?>" data-iblock-id="<?=$arResult["SKU_INFO"]["IBLOCK_ID"]?>" data-prop-id="<?=$arResult["SKU_INFO"]["SKU_PROPERTY_ID"]?>" data-hide-measure="<?=$arParams["HIDE_MEASURES"]?>" data-price-code="<?=implode("||", $arParams["PRODUCT_PRICE_CODE"])?>">
		...
		<div id="tableContainer">
			...
			<div id="elementContainer" class="column">
						...
						<div class="reviewsBtnWrap">
							<div class="row">
								<a class="label">
									...
									<span class="<?if(count($arResult["REVIEWS"]) > 0):?>countReviewsTools<?endif;?>"><?=GetMessage("REVIEWS_COUNT")?> <?=count($arResult["REVIEWS"])?></span>
									<div class="rating">
				...
				<?if(!empty($arResult["DETAIL_TEXT"]) || $arResult['PARENT_PRODUCT']['IPROPERTY_VALUES']['ELEMENT_PREVIEW_PICTURE_FILE_ALT']):?>
					<div id="detailText">
						<div class="heading"><?=GetMessage("CATALOG_ELEMENT_DETAIL_TEXT_HEADING")?></div>
						<div class="changeDescription" data-first-value='<?=str_replace("'", "", $arResult["~DETAIL_TEXT"])?>'>
						<?=$arResult['PARENT_PRODUCT']['IPROPERTY_VALUES']['ELEMENT_PREVIEW_PICTURE_FILE_ALT'] ? $arResult['PARENT_PRODUCT']['IPROPERTY_VALUES']['ELEMENT_PREVIEW_PICTURE_FILE_ALT'] : $arResult["~DETAIL_TEXT"]?></div>
					</div>
				<?endif;?>

		...
		<?if(!empty($arResult["PRICE"])):?>
					...
					<?if(!empty($arResult["PRICE"]["DISCOUNT"])):?>
						...
						<span class="oldPriceLabel"><s class="discount"><?=CCurrencyLang::CurrencyFormat($arResult["PROPERTIES"]["OLD_PRICE"]["VALUE"], $arResult["EXTRA_SETTINGS"]["CURRENCY"], true)?></s></span>
							<?if(!empty($arResult["PRICE"]["RESULT_PRICE"]["DISCOUNT"])):?>
								<span class="oldPriceLabel"><?=GetMessage("OLD_PRICE_DIFFERENCE_LABEL")?> <span class="economy"><?=CCurrencyLang::CurrencyFormat($arResult["PROPERTIES"]["OLD_PRICE"]["VALUE"], $arResult["EXTRA_SETTINGS"]["CURRENCY"], true)?></span></span>
							<?endif;?>
						</span>
					<?endif;?>
			...
			<?else:?>
				<a class="price changePrice">
					<?if(!empty($arResult["PRICE"]["DISCOUNT"]) || $arResult["PROPERTIES"]["OLD_PRICE"]["VALUE"]):?>
						<span class="priceBlock">
							<span class="oldPriceLabel"><s class="discount"><?=CCurrencyLang::CurrencyFormat($arResult["PROPERTIES"]["OLD_PRICE"]["VALUE"], $arResult["EXTRA_SETTINGS"]["CURRENCY"], true)?></s></span>
							<?if(!empty($arResult["PRICE"]["RESULT_PRICE"]["DISCOUNT"])):?>
								<span class="oldPriceLabel"><?=GetMessage("OLD_PRICE_DIFFERENCE_LABEL")?> <span class="economy"><?=CCurrencyLang::CurrencyFormat($arResult["PROPERTIES"]["OLD_PRICE"]["VALUE"], $arResult["EXTRA_SETTINGS"]["CURRENCY"], true)?></span></span>
				...
				<?if(!empty($arResult["PRICE"])):?>
					<?if($arResult["CATALOG_AVAILABLE"] != "Y"):?>
						<?if($arResult["CATALOG_SUBSCRIBE"] == "Y"):?>
							<a onclick="ym(21192229,'reachGoal','CLICK_OFFER');" href="#" class="addCart subscribe changeID changeQty changeCart" data-id="<?=$arResult["ID"]?>" data-quantity="<?=$arResult["EXTRA_SETTINGS"]["BASKET_STEP"]?>"><img src="<?=SITE_TEMPLATE_PATH?>/images/subscribe.png" alt="<?=GetMessage("SUBSCRIBE_LABEL")?>" class="icon"><?=GetMessage("SUBSCRIBE_LABEL")?></a>
						<?else:?>
							<a onclick="ym(21192229,'reachGoal','CLICK_OFFER');" href="/red.php?red=<?=urlencode('https://ad.admitad.com/g/17lxnyyakg5082102477c109c80522/?subid='.$arResult['ID'].'&ulp='.$arResult['PROPERTIES']['URL_PRODUCT']['VALUE'])?>" class="addCart changeID changeQty changeCart disabled" data-id="<?=$arResult["ID"]?>" data-quantity="<?=$arResult["EXTRA_SETTINGS"]["BASKET_STEP"]?>"><img src="<?=SITE_TEMPLATE_PATH?>/images/incart.png" alt="<?=GetMessage("ADDCART_LABEL")?>" class="icon">1<?=GetMessage("ADDCART_LABEL")?></a>
						<?endif;?>
					<?else:?>
						<a onclick="ym(21192229,'reachGoal','CLICK_OFFER');" target="_blank" href="/red.php?red=<?=urlencode('https://ad.admitad.com/g/17lxnyyakg5082102477c109c80522/?subid='.$arResult['ID'].'&ulp='.$arResult['PROPERTIES']['URL_PRODUCT']['VALUE'])?>" class="addCart" data-id="<?=$arResult["ID"]?>" data-quantity="<?=$arResult["EXTRA_SETTINGS"]["BASKET_STEP"]?>"><img src="<?=SITE_TEMPLATE_PATH?>/images/incart.png" alt="<?//=GetMessage("ADDCART_LABEL")?>" class="icon">Подробнее<?//=GetMessage("ADDCART_LABEL")?></a>
					<?endif;?>
				<?else:?>
					<a onclick="ym(21192229,'reachGoal','CLICK_OFFER');" href="/red.php?red=<?=urlencode('https://ad.admitad.com/g/17lxnyyakg5082102477c109c80522/?subid='.$arResult['ID'].'&ulp='.$arResult['PROPERTIES']['URL_PRODUCT']['VALUE'])?>" class="addCart changeID changeQty changeCart disabled requestPrice" data-id="<?=$arResult["ID"]?>" data-quantity="<?=$arResult["EXTRA_SETTINGS"]["BASKET_STEP"]?>"><img src="<?=SITE_TEMPLATE_PATH?>/images/request.png" alt="<?=GetMessage("REQUEST_PRICE_BUTTON_LABEL")?>" class="icon"><?=GetMessage("REQUEST_PRICE_BUTTON_LABEL")?></a>
				<?endif;?>
			</div>
			<div class="row columnRow">

		...
		<div class="tabloid nowp">
		    ...
		    <div class="productTable">
		    	...
		    	<div class="productColText">					
					<a href="<?=$arResult["DETAIL_PAGE_URL"]?>" class="name"><span class="middle"><?= (IS_FILTER && $arResult["PROPERTIES"]["SHORT_NAME"]["VALUE"] ? /* $APPLICATION->ShowViewContent('sotbit_seometa_bread') . " " .  */$arResult["BRAND"] . " " . $arResult["PROPERTIES"]["SHORT_NAME"]["VALUE"] : $arResult["NAME"])  ?></span></a>
					<?if(!empty($arResult["PRICE"])):?>
						...
						<?if($arResult["EXTRA_SETTINGS"]["COUNT_PRICES"] > 1):?>
							<a class="price getPricesWindow" data-id="<?=$arResult["ID"]?>">
								<span class="priceIcon"></span>Цена: <?=CCurrencyLang::CurrencyFormat($arResult["PRICE"]["DISCOUNT_PRICE"], $arResult["EXTRA_SETTINGS"]["CURRENCY"], true)?>
						...
						<?else:?>
							<a class="price">Цена: <?=CCurrencyLang::CurrencyFormat($arResult["PRICE"]["DISCOUNT_PRICE"], $arResult["EXTRA_SETTINGS"]["CURRENCY"], true)?>
								<?if($arParams["HIDE_MEASURES"] != "Y" && !empty($arResult["EXTRA_SETTINGS"]["MEASURES"][$arResult["CATALOG_MEASURE"]]["SYMBOL_RUS"])):?>
									<span class="measure"> / <?=$arResult["EXTRA_SETTINGS"]["MEASURES"][$arResult["CATALOG_MEASURE"]]["SYMBOL_RUS"]?></span>
								<?endif;?>
								<s class="discount">
					...
					<div class="addCartContainer">
						<?if(!empty($arResult["PRICE"])):?>
							...
							<?endif;?>
						<?else:?>
							<a onclick="ym(21192229,'reachGoal','CLICK_OFFER');" target="_blank" href="/red.php?red=<?=urlencode('https://ad.admitad.com/g/17lxnyyakg5082102477c109c80522/?subid=' . $arResult['ID'] . '&ulp=' . $arResult['PROPERTIES']['URL_PRODUCT']['VALUE'])?>" class="addCart disabled requestPrice" data-id="<?=$arResult["ID"]?>"><span><img src="<?=SITE_TEMPLATE_PATH?>/images/request.svg" alt="" class="icon">Подробнее</span></a>
						<?endif;?>
					</div>

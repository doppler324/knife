								...
								<?foreach($arItem["VALUES"] as $val => $ar):?><?$i++;?>
									<li class="<?=($i > 5) ? "off" : ""?>">
										...
										<label for="<?=$ar["CONTROL_ID"]?>" data-role="label_<?=$ar["CONTROL_ID"]?>">
											<?if($ar["URL_FOR_FILTER"] && $ar["DISABLED"] != 1):?><a href="<?=$ar["URL_FOR_FILTER"]?>"><?endif;?>
											<?=$arParams["TAG_META_HEADING"] ? str_replace("{plus_filter}","",$arParams["TAG_META_HEADING"]) : $arResult['SECTION_TITLE']?><?=$arItem["FILTER_HINT"] ? ' '.$arItem["FILTER_HINT"] :''?> <?=$ar["VALUE"]?>
											<?if ($arParams["DISPLAY_ELEMENT_COUNT"] !== "N" && isset($ar["ELEMENT_COUNT"])):?>
												<ins class="elCount" data-role="count_<?=$ar["CONTROL_ID"]?>">(<?=$ar["ELEMENT_COUNT"]?>)</ins>
											<?endif;?>
											<?if($ar["URL_FOR_FILTER"] && $ar["DISABLED"] != 1 && stripos($APPLICATION->GetCurPage(false), "/tag/") !== false):?></a><?endif;?>
										</label>
									</li>
								<?endforeach;?>

		    	...
		    	<div class="productColImage">
					<a href="<?=$arResult["DETAIL_PAGE_URL"]?>" class="picture">
						<?if($arParams["LAZY_LOAD_PICTURES"] == "Y"):?>
							<img src="<?=SITE_TEMPLATE_PATH?>/images/lazy.svg" class="lazy" data-lazy="<?=$arResult["PICTURE"]["src"]?>" alt="<?=(IS_FILTER && $arResult["PROPERTIES"]["SHORT_NAME"]["VALUE"] ? $APPLICATION->ShowViewContent('sotbit_seometa_bread') . " " . $arResult["BRAND"] . " " . $arResult["PROPERTIES"]["SHORT_NAME"]["VALUE"] : $arResult["NAME"])?>" title="<?=(IS_FILTER && $arResult["PROPERTIES"]["SHORT_NAME"]["VALUE"] ? $APPLICATION->ShowViewContent('sotbit_seometa_bread') . " " . $arResult["BRAND"] . " " . $arResult["PROPERTIES"]["SHORT_NAME"]["VALUE"] : $arResult["NAME"])?>">
						<?else:?>
							<img src="<?=$arResult["PICTURE"]["src"]?>" alt="<?=(IS_FILTER && $arResult["PROPERTIES"]["SHORT_NAME"]["VALUE"] ? $APPLICATION->ShowViewContent('sotbit_seometa_bread') . " " . $arResult["BRAND"] . " " . $arResult["PROPERTIES"]["SHORT_NAME"]["VALUE"] : $arResult["NAME"])?>" title="<?=(IS_FILTER && $arResult["PROPERTIES"]["SHORT_NAME"]["VALUE"] ? $APPLICATION->ShowViewContent('sotbit_seometa_bread') . " " . $arResult["BRAND"] . " " . $arResult["PROPERTIES"]["SHORT_NAME"]["VALUE"] : $arResult["NAME"])?>">
						<?endif;?>
						<span class="getFastView" data-id="<?=$arResult["ID"]?>"><?=GetMessage("FAST_VIEW_PRODUCT_LABEL")?></span>
					</a>
		    	</div>
		    	<div class="productColText">
					<a href="<?=$arResult["DETAIL_PAGE_URL"]?>" class="name"><span class="middle"><?=(IS_FILTER && $arResult["PROPERTIES"]["SHORT_NAME"]["VALUE"] && !IS_BRANDS_PAGE ? $APPLICATION->ShowViewContent('sotbit_seometa_bread') . " " . $arResult["BRAND"] . " " . $arResult["PROPERTIES"]["SHORT_NAME"]["VALUE"] : $arResult["NAME"])?></span></a>					
					<?if(!empty($arResult["PRICE"])):?>
						<?//price container?>
						<?if($arResult["EXTRA_SETTINGS"]["COUNT_PRICES"] > 1):?>
							<a class="price getPricesWindow" data-id="<?=$arResult["ID"]?>">
								<span class="priceIcon"></span>Цена: <?=CCurrencyLang::CurrencyFormat($arResult["PRICE"]["RESULT_PRICE"]["DISCOUNT_PRICE"], $arResult["EXTRA_SETTINGS"]["CURRENCY"], true)?>
								
								<s class="discount">
									<?if(!empty($arResult["PRICE"]["RESULT_PRICE"]["BASE_PRICE"])):?>
						...
						<?else:?>
							...
							<a class="price">Цена: <?=CCurrencyLang::CurrencyFormat($arResult["PRICE"]["RESULT_PRICE"]["DISCOUNT_PRICE"], $arResult["EXTRA_SETTINGS"]["CURRENCY"], true)?>
					...
					<div class="propContainer">					
						<?foreach($arResult['PROPERTIES'] as $key => $item):?>
							<?if(in_array($item['ID'], $arResult['PROPERTIES']['ID_PROP_SECTION']['VALUE']) && $item["VALUE"]):?>
								<span><?=$item["NAME"]?></span> : <?=is_array($item["VALUE"]) ? implode(', ', $item["VALUE"]) : $item["VALUE"]?><br>
							<?endif;?>
						<?endforeach;?>
					</div>
			...
			<div class="optional">
				...
				<div class="row">
					...
					<?if($arResult["CATALOG_QUANTITY"] > 0):?>
						...
						<?else:?>
							<span class="inStock label changeAvailable"><img src="<?=SITE_TEMPLATE_PATH?>/images/inStock.png" class="icon"><span><?=GetMessage("AVAILABLE")?></span></span>
						<?endif;?>
					<?else:?>
						<?if($arResult["CATALOG_AVAILABLE"] == "Y"):?>

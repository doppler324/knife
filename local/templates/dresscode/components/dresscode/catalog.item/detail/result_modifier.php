<?
	...
	if(!empty($arResult)){
		...
		if(!empty($arParams["REVIEW_IBLOCK_ID"])){
			...
			if(empty($arParams["ALLOW_ADD_REVIEW_NOT_REGISTER"]) || $arParams["ALLOW_ADD_REVIEW_NOT_REGISTER"] != "Y"){
		...
		//line breaks
		if(!empty($arResult["DETAIL_TEXT"]) && !empty($arResult["DETAIL_TEXT_TYPE"]) && $arResult["DETAIL_TEXT_TYPE"] == "text"){
...
<?/*if($_SERVER['REQUEST_URI'] !== $arResult['DETAIL_PAGE_URL']  && !CUser::IsAdmin() ){
   LocalRedirect ($arResult['DETAIL_PAGE_URL'], false, "301 Moved permanently");   
}*/?>

<?
$res = CIBlockSection::GetList(array(), array("IBLOCK_ID" => $arResult["SECTION"]["IBLOCK_ID"], "ID" => $arResult["SECTION"]["ID"]), false, array("UF_BEFORE_PRODUCT"));
if($arRes = $res->GetNext()){
	$section = $arRes["UF_BEFORE_PRODUCT"];
}unset($res);
$arResult["NAME"] = $arResult['PROPERTIES']["SHORT_NAME"]['VALUE'] ? $section . " " . $arResult["BRAND"]["NAME"] . " " . $arResult['PROPERTIES']["SHORT_NAME"]['VALUE'] : $arResult["NAME"];unset($section);

...
if(!$arUrlParams['spec'] && !$arUrlParams['review'] && !$arUrlParams['comment']){
	$template = "detail";
}elseif($arUrlParams['spec']){
	$template = "detail_char";
}elseif($arUrlParams['review'] ){
	$template = "detail_review";
}elseif($arUrlParams['comment']){
	$template = "detail_comment";
}
...
<?$APPLICATION->IncludeComponent(
	...
	false,
	array("HIDE_ICONS" => "Y")
);unset($template);?><?$this->EndViewTarget();?>